...
async function nextRange(apiKey: string, offer: string, copyCode: string, reservedMax = 0) {
  ...
  const res = await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(name)&pageSize=1000&key=${encodeURIComponent(apiKey)}`);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { action, date, demands } = await req.json();
    if (!["preview", "create"].includes(action) || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !Array.isArray(demands)) return json({ error: "Pedido inválido." }, 400);
    ...
    if (demands.some((d) => !allowedCopies.has(d.copyCode) || !allowedOffers.has(d.offer) || d.date !== date)) return json({ error: "Demanda não permitida." }, 400);
    const driveApiKey = Deno.env.get("GOOGLE_DRIVE_API_KEY");
    if (!driveApiKey) throw new Error("Google Drive ainda não configurado.");
    const { data: existing } = await supabase.from("integration_sync_log").select("copy_code,clickup_task_id,status,task_name").eq("user_id", user.id).eq("sync_date", date);
    const { data: reservations } = await supabase.from("integration_sync_log").select("copy_code,offer_code,range_end,status").eq("user_id", user.id).in("status", ["pending", "created"]);
    const items = [];
